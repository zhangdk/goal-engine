...
## 1. 项目愿景 (Vision)
`goal-engine` 是一个专为自主 Agent 设计的**目标治理与行为演进系统**。它通过中心化的后端服务与分布式的执行挂架（Harness），解决了当前 AI Agent 最致命的三个痛点：**目标漂移（Goal Drift）**、**失败即放弃（Fragility）**以及**能力停滞（Static Capability）**。

在 `goal-engine` 的逻辑中，任务不是一次性的 Prompt 交互，而是一个**受控的、可反思的、且必须被达成的生命周期**。

---

## 2. 核心架构：中心化治理 + 分布式压榨 (Architecture)

![Distributed Swarm Architecture](docs/images/arch_distributed_swarm.svg)
